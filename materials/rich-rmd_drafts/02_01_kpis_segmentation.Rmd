...
Let's use the `daily_users` table to make functions that get us each of these KPIs. First, we need access to the table in the database:

```{r}
daily_users <- "<<FUNCTION>>"
```
...
Just like ARPU, we often don't care about DAU and DAC on specific days but for certain spans of spans of time. These spans of time could be:

- days of certain months
- spans of 15, 30, 60, 90, 120, 180, etc. days back from a certain date
- a specific set of days between significant dates

The task is to write functions for calculating DAU, MAU, DAC, and ARPU for user-defined spans of time. Create a common interface for each one. Each should return a numeric value.
