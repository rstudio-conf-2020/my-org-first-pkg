...
Open up the package-in-progress `materials/day_2/p_01_intendo/01_intendo.Rproj`.

Take a look around and see which package components are in place.

There are no functions yet (No R folder!) but there is a `scripts` folder with .R files that contain functions that have been tested out locally. Test them yourself, then, integrate them into the package.

Some **usethis** functions to try out:
...
Occasionally use `R CMD check` to see if there are any problems (and then fix those problems).

Once the functions are integrated into the package. Build the package and test out the new package functions! If there's time, create a `NEWS.md` file, `README.Rmd/README.md`, and some function
documentation.
